      ...
      {products.map((product, index) => {
        ...
        return (
          <Link key={index} to={`/product/${product._id}`} className='block group'>
              <div className={`bg-white p-4 rounded-2xl shadow-lg hover:shadow-xl transition-all duration-500 ease-out group-hover:scale-105 ${isOutOfStock ? 'opacity-75' : ''}`}>
                  <div className="w-full h-96 mb-4 overflow-hidden rounded-xl relative">
                      <LazyImage 
