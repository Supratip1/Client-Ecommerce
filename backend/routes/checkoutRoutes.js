...
router.post("/:id/finalize", protect, async (req, res) => {
  try {
    ...
    if (checkout.isPaid && !checkout.isFinalized) {
      // Order is already created by StripeButton, just mark checkout as finalized
      // No need to create another order - this prevents duplicates
      checkout.isFinalized = true;
      checkout.finalizedAt = Date.now();
      await checkout.save();
      
      // Find the existing order created by StripeButton
